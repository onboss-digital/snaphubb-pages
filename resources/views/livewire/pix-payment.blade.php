<div>
    ...
    <div id="pix-modal"
        ...
        <div class="bg-[#1F1F1F] rounded-xl max-w-3xl w-full mx-auto p-6 md:p-8 z-10 relative overflow-y-auto max-h-full" @click.stop>
            ...
            <div x-show="!$wire.pixQrCodeBase64 && !$wire.showLodingModal" x-transition>
                <div class="flex flex-col md:flex-row gap-6 md:gap-8">
                    ...
                    <div class="w-full md:w-1/2 order-2 md:order-1">
                        <h3 class="text-xl md:text-2xl font-bold text-white mb-2">{{ __('payment.pix_title') }}</h3>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input name="pix_cpf" type="text" placeholder="{{ __('payment.cpf_placeholder') }}" wire:model.defer="pix_cpf" class="w-full bg-[#2D2D2D] text-white rounded-lg p-3 border @error('pix_cpf') border-red-500 @else border-gray-700 @enderror focus:outline-none focus:ring-1 focus:ring-[#E50914] transition-all" />
                ...
                <div class="mt-8 flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                    <button @click="show = false; $wire.call('closeModal')" class="w-full sm:w-auto py-2 px-6 text-white font-medium rounded-lg border border-gray-600 hover:bg-[#2D2D2D] transition-colors">
                        {{ __('payment.pix_cancel_button') }}
                    </button>
                    <button wire:click="startPixCheckout" wire:loading.attr="disabled" wire:loading.class="opacity-50" class="w-full sm:w-auto py-3 px-8 bg-[#E50914] hover:bg-[#B8070F] text-white font-bold rounded-lg transition-colors text-lg flex items-center justify-center">
                        <span wire:loading.remove wire:target="startPixCheckout">{{ __('payment.generate_pix_button') }}</span>
                        <span wire:loading wire:target="startPixCheckout" class="flex items-center">{{ __('payment.generating_pix_button') }}
            ...
            <!-- Step 2: QR Code -->
            <div x-show="$wire.pixQrCodeBase64 && !$wire.showLodingModal" x-transition class="text-center flex flex-col items-center">
                <h3 class="text-xl md:text-2xl font-bold text-white mb-2">{{ __('payment.pix_generated_title') }}</h3>
                <p class="text-gray-300 mb-4 text-sm md:text-base max-w-md">{{ __('payment.pix_instructions') }}</p>

                <div class="flex justify-center mb-4 w-full max-w-xs">
                    <img :src="'data:image/png;base64,' + $wire.pixQrCodeBase64" alt="{{ __('payment.pix_qr_code_alt') }}" class="rounded-lg border-4 border-white w-full h-auto">
                </div>
                ...
                <div class="mb-4 w-full max-w-sm mx-auto">
                    ...
                    <button @click="navigator.clipboard.writeText($wire.pixQrCode); $event.target.innerText = '{{ __('payment.copied_button') }}'; setTimeout(() => $event.target.innerText = '{{ __('payment.copy_code_button') }}', 2000)"
                        class="mt-2 bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded transition-all duration-300 w-full">
                ...
                <button @click="show = false; $wire.call('closeModal')" class="mt-4 text-gray-400 hover:text-white text-sm">{{ __('payment.pix_pay_with_card_button') }}</button>
            </div>
        </div>
    </div>
