...
<body class="bg-black text-white min-h-screen">
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-6 py-12 md:py-24">
            ...
            <div class="space-y-6 mb-8 pb-8 border-b border-gray-800">
                ...
                <div class="flex justify-between items-center p-4 bg-red-600/10 rounded-lg border border-red-600/30">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="font-bold text-white">{{ __('upsell.voting_plan') }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6 hover:border-red-600/40 transition-all">
                ...
                <h4 class="font-bold text-white mb-2">{{ __('upsell.access_your_account') }}</h4>
                <p class="text-gray-400 text-sm">{{ __('upsell.access_account_desc') }}</p>
            </div>
        ...
        <div class="bg-gradient-to-r from-red-600/20 to-red-600/10 border border-red-600/40 rounded-2xl p-8 mb-12">
            <h3 class="text-2xl font-bold mb-6">{{ __('upsell.what_you_unlocked') }}</h3>
            <div class="space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-gray-300 font-semibold">{{ __('upsell.benefit_1') }}</p>
                        <p class="text-gray-400 text-sm">{{ __('upsell.benefit_1_desc') }}</p>
                    </div>
                </div>
                <div class="flex items-start gap-3">
                    <svg class="w-5 h-5 text-green-500 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        <p class="text-gray-300 font-semibold">{{ __('upsell.benefit_2') }}</p>
                        <p class="text-gray-400 text-sm">{{ __('upsell.benefit_2_desc') }}</p>
                    </div>
                </div>
                <div class="flex items-start gap-3">
                    <svg class="w-5 h-5 text-green-500 flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        <p class="text-gray-300 font-semibold">{{ __('upsell.benefit_3') }}</p>
                        <p class="text-gray-400 text-sm">{{ __('upsell.benefit_3_desc') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-8">
            <h3 class="text-xl font-bold mb-4">{{ __('upsell.need_help') }}</h3>
            <p class="text-gray-400 mb-6">{{ __('upsell.support_available') }}</p>
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                >
                    ...
                    <span id="email-text">{{ __('upsell.support_email') }}</span>
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        // Copy email function
        function copyEmail() {
            const email = '{{ __('upsell.support_email') }}';
            navigator.clipboard.writeText(email).then(() => {
                ...
                text.textContent = '{{ __('upsell.copy_email') }}';
                
                setTimeout(() => {
                    btn.classList.remove('bg-green-600');
                    btn.classList.add('bg-gray-800', 'hover:bg-gray-700');
                    text.textContent = '{{ __('upsell.support_email') }}';
                }, 2000);
            });
        }
    </script>
